<?php 
   ...
   $rea = $conn->query("SELECT users.*,years.* FROM users LEFT JOIN years ON users.year_id = years.years_id ORDER BY userid ASC");	
   foreach ($rea as $king => $react) {
    ...
    $output = '<tr>
              <td><img src="../admin/userprofiles/'.$react['userphoto'].'" style="height:25px; width:25px"></td>
    				  <td>'.$react['username'].'<i style="text-align:right"> &nbsp;'.count_unseen_message($react['userid'],$_SESSION['admin_id']).'</i></td>
              <td>'.$out.'</td>
    				  <td>'.$status.'</td>
    				  <td><a href="chat-private.php?userid='.$react['userid'].'" name="chat" data-id='.$react['userid'].' id="chatt" class="btn btn-info chatt"><i class="fa fa-location-arrow"></i> &nbsp;Start Chat</a>
    				  </td>
    				  </tr>';
            echo $output;
   } 
